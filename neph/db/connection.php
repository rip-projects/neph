<?php namespace Neph\DB;

use PDO, PDOStatement, Neph\Config, Neph\Event, Neph\Console;
...
class Connection {
	...
	protected function grammar()
	{
		...
		if (isset(\Neph\DB::$registrar[$this->driver()]))
		{
			return $this->grammar = \Neph\DB::$registrar[$this->driver()]['query']();
		}

		$grammar_class = Config::get('db/drivers/'.$this->driver(), '\\Neph\\DB\\'.$this->driver()).'\\Grammar';
	...
	protected function log($sql, $bindings, $start)
	{
		...
		Event::fire('Neph.query', array($sql, $bindings, $time));
